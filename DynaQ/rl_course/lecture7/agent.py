...
#         self.model = {}
#         self.visited_states = {}
#         self.timestep_table = np.zeros((self.states_n, self.actions_n))
...
#     def update_q(self, state, action, next_state, reward):
#         self._update(state, action, next_state, reward)
#         exploratory_bonus = self.kappa * np.sqrt(self.timestep_table[state, action])
#         self.q_table[state, action] = self.q_table[state, action] + self.alpha * (
#             reward + exploratory_bonus
#             + self.gamma * np.max(self.q_table[next_state])
#             - self.q_table[state, action]
#         )
#         self.timestep_table[state, action] = 0
#         self.timestep_table += 1
